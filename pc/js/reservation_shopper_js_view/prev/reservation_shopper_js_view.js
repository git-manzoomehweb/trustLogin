...
function renderDataBuyer(e,t,l){let i="";if("code"==t){for(let a=0;a<e.length;a++)i+=`<li onclick="select_code_val(this)" class="li-item" data-dial_code="${e[a].dial_code}" data-code="${e[a].code}"> (${e[a].dial_code}) ${e[a].name}</li>`;if("default"==l)for(var c=0;c<document.querySelector(".buyer-info-content").getElementsByClassName("package-number-items").length;c++)document.querySelector(".buyer-info-content").getElementsByClassName("package-number-items")[c].querySelector(".drop-item").innerHTML=i;else l.closest(".package-code-item").querySelector(".drop-item").innerHTML=i}else if("agency"==t){for(let n=0;n<e.length;n++)i+=`<li onclick="select_agency(this)" class="li-item">
