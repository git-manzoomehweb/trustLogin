...
function setDmnToken() {
    ...
    const thisdmntoken = document.querySelector("#dmntoken").value;
...
async function onProcessedToken(args) {
    ...
    if (errorid == '1') {
        ...
        if (token) {
            ...
            const loginCode = loginForm?.querySelector(".id-loginCode")?.value || "";
            const usernameInput = loginForm?.querySelector("input[name='username']")?.value || "";

            console.log(loginCode);
            console.log(usernameInput);

            const username = loginCode + usernameInput;
 ...
 <path d="M28.64 18.35C27.19 16.9 25.2 16 22.99 16C18.57 16 15 19.58 15 24C15 28.42 18.57 32 22.99 32C26.72 32 29.83 29.45 30.72 26H28.64C27.82 28.33 25.6 30 22.99 30C19.68 30 16.99 27.31 16.99 24C16.99 20.69 19.68 18 22.99 18C24.65 18 26.13 18.69 27.21 19.78L23.99 23H30.99V16L28.64 18.35Z" fill="#004B85"/>
            ...
            itemTempCopyForSubmitBtn.innerHTML = `
                ...
                <button type="submit" class="login-btn background-color_1 bg-primary">
                  ${culture === 'fa'
                    ? 'مرحله بعدی'
                    : culture === 'ar'
                        ? 'التالي'
...
async function onProcessedCheckrkey(args) {
    ...
    if (responseJson.checked == true) {
        ...
        if (document.querySelector(".main-userid")) {
            ...
            const previousPassengersGrid = document.querySelector('.previous__passengers__container');
            if (previousPassengersGrid) {
                if (passengerList_key == '1') {
                    $bc.setSource("cms.previousPassengers", { rkey: responseJson.rkey });
                    const passengerInfoElements = document.getElementsByClassName("passenger-info-content");
                    Array.from(passengerInfoElements).forEach(infoContent => {
                        if (!infoContent.parentElement.classList.contains("hidden")) {
                            const passengers = infoContent.getElementsByClassName("passenger");
                            if (passengers.length > 0) {
                                Array.from(passengers).forEach(passenger => {
                                    passenger.querySelector(".prev-passengers").setAttribute("data-clicked", 1);
                                    passenger.querySelector(".prev-passengers").classList.remove("selectedPassenger")
                                });
                            } else {
                                infoContent.querySelector(".prev-passengers").setAttribute("data-clicked", 1);
                                infoContent.classList.remove("selectedPassenger")
                            }
                            if (infoContent.dataset.index == index_key) {
                                infoContent.classList.add("selectedPassenger")
                            }
                        }
...
try {
    ...
    if (
        document.querySelector(".company-ownerid").value == '9364' ||
        document.querySelector(".company-ownerid").value == '9453' ||
        document.querySelector(".company-ownerid").value == '9508' ||
        document.querySelector(".company-ownerid").value == '9599' ||
        document.querySelector(".company-ownerid").value == '9481' ||
        document.querySelector(".company-ownerid").value == '9474' ||
        document.querySelector(".company-ownerid").value == '9474' ||
        document.querySelector(".company-ownerid").value == '9472' ||
...
function detectInputType(element) {
    let inputElement = element.value;
...
function selectCountryLoginCode(element) {
    element.closest("form").querySelector(".id-loginCode").value = element.dataset.id;
